...
class LightweightEncoder(nn.Module):
	...
	def forward(self, x, thresh=0.5):
		...
		data_com = self.pool1(x)[mask]
		not_cond = torch.logical_not(cond)
		data_raw = x[not_cond]
		data_out = torch.cat((data_raw,data_com),0)
		# compressed data in the original shape
		if not self.training:
			x = torch.where(cond, ss, x)
		else:
			x = torch.mul(x,feat_) + torch.mul(ss,1-feat_)
		...
		quantized_x = softout
		data_out = data_out.view(*(list(data_out.size()) + [1]))
		quant_dist = torch.pow(data_out-self.centers, 2)
		...
		rle_len1 = mask_compression(mask.view(-1).cpu().numpy())
		softmax_dist = nn.functional.softmax(-quant_dist, dim=-1)
		soft_prob = torch.mean(softmax_dist,dim=0)
		entropy_loss = -torch.sum(torch.mul(soft_prob,torch.log(soft_prob)))
		real_size += rle_len1
		filter_loss = torch.mean(feat)
		real_com_rate = 1/16.*real_size/(H*W*C*B*8)
		loss = filter_loss + entropy_loss

		return quantized_x,real_com_rate,loss

class DCCOI(nn.Module):

	def __init__(self, kernel_size=4, num_centers=8):
		super(DCCOI, self).__init__()
		out_size = 3
		no_of_hidden_units = 64
		self.encoder = LightweightEncoder(out_size, kernel_size=4, num_centers=8)
		self.conv1 = ConvLayer(out_size,out_size)
		self.resblock_up1 = Resblock_up(out_size,no_of_hidden_units)
		self.conv2 = ConvLayer(no_of_hidden_units,no_of_hidden_units)
		self.resblock_up2 = Resblock_up(no_of_hidden_units,no_of_hidden_units)
		self.OutputConv = OutputConv(no_of_hidden_units)
		
	def forward(self, x):
		x,r,loss = self.encoder(x)
